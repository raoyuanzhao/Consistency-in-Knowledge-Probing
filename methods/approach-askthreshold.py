...
if __name__ == "__main__":
    ...
    with open("data/" + dataset + ".json", "r") as f:
        ...
        if best_threshold<0.2 or best_threshold>0.8:
            best_threshold = 0.5
    ...
    folder_path = f'Probing_Uncertainy/{model_name}_{dataset}'

    # Create the folder if it doesn't exist
    os.makedirs(folder_path, exist_ok=True)
    output_file = f'Probing_Uncertainy/result/{model_name}_{dataset}/calibration_threshold_{model_name}_{dataset}_setup_{prompt_index}.json'

    with open(output_file, "w") as file:
        json.dump(results, file, indent=4)

    with open(f'Probing_Uncertainy/pickle_result/calibration_threshold_{model_name}_{dataset}_setup_{prompt_index}_result.pkl', 'wb') as f:
