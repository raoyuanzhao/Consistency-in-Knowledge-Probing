...
def llm_init(model_name):
    ...
    if model_name == "olmo":
        device = "cuda"
        model = AutoModelForCausalLM.from_pretrained("allenai/OLMo-2-1124-7B-Instruct", device_map="auto", torch_dtype=torch.bfloat16)
        tokenizer = AutoTokenizer.from_pretrained("allenai/OLMo-2-1124-7B-Instruct")
    
    if model_name == "270b":
        device = "cuda"
        model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.1-70B-Instruct", device_map="auto", torch_dtype=torch.bfloat16, cache_dir="/mounts/Users/cisintern/rzhao/storage/storage/model")
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-70B-Instruct", cache_dir="/mounts/Users/cisintern/rzhao/storage/storage/model")
        #model.to(device)
    if model_name == "3b":
        device = "cuda"
        model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-3B-Instruct", device_map="auto", torch_dtype=torch.bfloat16, cache_dir="")
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-3B-Instruct", cache_dir="")
    if model_name == "1b":
        device = "cuda"
        model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-1B-Instruct", device_map="auto", torch_dtype=torch.bfloat16, cache_dir="")
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-1B-Instruct", cache_dir="")
    if model_name == "8b":
        device = "cuda"
        model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-8B-Instruct", device_map="auto", torch_dtype=torch.bfloat16, cache_dir="")
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-8B-Instruct", cache_dir="")
    if model_name == "70b":
        device = "cuda"
        model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.1-70B-Instruct", device_map="auto", torch_dtype=torch.bfloat16, cache_dir="")
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-70B-Instruct", cache_dir="")
...
def llm_response(prompt, model_name, probs = False, logit = False, temperature = 0.1, max_new_tokens = 200):
    if model_name == "mistral" or model_name == "llama3" or model_name == "70b" or model_name == "270b" or model_name == "3b_1" or model_name == "1b" or model_name == "8b" or model_name == "llama3_70b":
        messages = [
        ...
        # Special handling for OLMo Instruct with its specific chat template
        formatted_prompt = f"<|endoftext|><|user|>\n{prompt}\n<|assistant|>\n"
        ...
        # Handle token-level generation probabilities
        if probs:
            transition_scores = model.compute_transition_scores(
                outputs.sequences, outputs.scores, normalize_logits=True
            )
            for tok, score in zip(generated_ids[0], transition_scores[0]):
                ...
                token_probs[token] = np.exp(score.item())

        # Handle first-token logits for A/B/C/D
        if logit and outputs.scores:
            
            logits = outputs.scores[0][0]  # logits for first generated token: shape [vocab_size]

            target_tokens = ["A", "B", "C", "D"]
            token_ids = [tokenizer.encode(t, add_special_tokens=False)[0] for t in target_tokens]

            selected_logits = logits[token_ids]                    # Tensor of 4 values
            selected_probs = F.softmax(selected_logits, dim=-1)   # Softmax over A/B/C/D only

            first_token_logits = {
                tok: selected_probs[i].item() for i, tok in enumerate(target_tokens)
            }
        ...
        if probs and logit:
            return decoded[0], token_probs, first_token_logits
        elif probs:
            return decoded[0], token_probs
        elif logit:
            return decoded[0], first_token_logits
        else:
            return decoded[0]
